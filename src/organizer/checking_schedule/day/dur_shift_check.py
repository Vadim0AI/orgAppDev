...
def dur_shift_check(old_schedule: list[tuple], new_schedule: list[tuple], all_wb: dict[tuple]) -> list[bool, str]:
    ...
    for unique_wb in new_unique_wb:
        ...
        # Получаем настройку в виде строки.
        wb_setting_str = all_wb[unique_wb][6]
        # Если в настройке ничего нет т.е. указано '0.0' или '0' или '_ |
        # _', то пропускаем проверку по этому РБ.
        if (wb_setting_str != '0') and (wb_setting_str != '0.0') and (
                wb_setting_str != '_ | _'):
            # Преобразуем настройку в словарь.
            wb_setting = get_wb_settings_dct(wb_setting_str)
